      ...
      <div ref={contentRef} className={`flex w-max pr-[2px]`}>
        ...
        // {/* {[...projects].filter((project) => (project.images?.length ?? 0) > 1).map((project, index) => ( */}
          ...
          <div
            ...
            className={`sticky left-0 duration-666 bg-(--color-positive) lg:hover:pt-[calc(var(--lh)+4px)] ${activeCategorySlug && project.categories?.[0]?.slug !== activeCategorySlug ? 'w-0 p-0! duration-0! overflow-hidden' : ''}`}

...
function story() {
	...
	$('.gallery-item, .selected-item, #single-gallery').each(function () {
		...
		const item = $(this);
		const isSingleGallery = item.is('#single-gallery');
		...
		let current = item.data('current') || 1;
		...
		// caption hover
		caption.on('mouseenter', () => captionActive = true);
		caption.on('mouseleave', () => captionActive = false);
		...
		const startAutoplay = () => {
			...
			const interval = setInterval(() => {
				...
				current = (item.data('current') % total) + 1;
				activate(current);
		...
		if (!isSingleGallery) {
			...
			item.on('mouseenter', startAutoplay);

			item.on('mouseleave click', function (e) {
				if (e.type === 'click' && $('#front-page').attr('data-template') === 'single') return;
				pauseAutoplay();
			});
		...
		if (isSingleGallery) {
			...
			item.on('mouseenter', pauseAutoplay);
			item.on('mouseleave', startAutoplay);

			mediaIndex.on('mouseleave', startAutoplay);
		}
	});
}

function resetStory(item) {

	const interval = item.data('interval');
	if (interval) {
		clearInterval(interval);
		item.removeData('interval');
	}

	item.data('current', 1);

	item.find('img[data-index], .media-index [data-index]')
		.removeClass('active');

	item.find('[data-index="1"]').addClass('active');
}
...
$(document).on('mouseenter', '.archive-item', function () {
	...
	if ($('#categories a').hasClass('filter') || $('#front-page>*').hasClass('hide')) return;
...
function showProject(slug) {
	...
	function renderMedia(mediaContainer, project) {
		...
		$('#all-button').on('click', function () {
			resetStory(singleGallery); 
			archiveGallery.removeClass('hide');
			singleGallery.removeClass('active');

			archiveGallery.children().removeClass('active');
			archiveGallery.scrollTop(0);
		});
